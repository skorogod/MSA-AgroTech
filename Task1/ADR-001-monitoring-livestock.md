...
- **Агент** — локальный сервер фермы вместе с его ПО. Управляет edge-устройствами: камерами, кормушками, поилками, фильтрами. Количество агентов равно количеству ферм.
...
Выбрано решение **«локальный сервер-агент на ферме + одна центральная система»**. Видеоаналитика, оповещения и хранение данных выполняются на ферме. Центральная система собирает события и метрики со всех ферм и предоставляет API.
...
- Оборудование разных производителей (ФТ 3, 8) подключается через слой адаптеров в составе агента.
...
- Логика животноводства не размещается в ERP и SCADA, так как это ограничит расширяемость (НФТ 2) и последующий вынос платформы в SaaS.
